...
char* invertirCase(char* str) { // paso test
...
{
	if (!largoVecStr) return NULL;

	char** ret = new char*[largoVecStr];

	bool* indicesUsados = new bool[largoVecStr]; // array para ir marcando los indices que ya movi, para hacer como un inserction sort
	for (int i = 0; i < largoVecStr; i++) {
	...
	while (indiceDestino < largoVecStr) {
		int indicePalabraMenor = -1;

		for (int i = 0; i < largoVecStr; i++) {
			bool indiceUsado = indicesUsados[i];
			if (indicePalabraMenor == -1 && !indiceUsado) indicePalabraMenor = i;
			if (!indiceUsado && ordenStr(vecStr[i], vecStr[indicePalabraMenor])) indicePalabraMenor = i;
		}

		ret[indiceDestino] = new char[largoString(vecStr[indicePalabraMenor])];
		copiarString(vecStr[indicePalabraMenor], ret[indiceDestino]);
		indicesUsados[indicePalabraMenor] = true;
		indiceDestino++;
	}
