...
bool PruebaOcurrenciasSubstring(const char *stringVecStr, const char *substr, int output, int &nroPrueba){
	...
	unsigned int res = ocurrenciasSubstring(vecStr, largoVecStr, substringCopiaPasado);

	bool resultadoOK = (int)res == output;
	...
	if (!resultadoOK) {
		cout << " -> ERROR salida incorrecta" << endl;
		FrameworkA1::imprimirEsperadoVsRecibido(output, (int)res, inputParametersFormated);
	} else if(modificoParametros) {
		cout << " -> ERROR modifico parametros" << endl;
		resultadoOK = false;
	} else
